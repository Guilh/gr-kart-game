...
**Session 3 status (2026-09-24):** launch prep. Section 9 (thermal and performance) is done, section 7 hygiene is done except unit checks and git, and 10.1 (build, preview, smoke test) is done. Git: `main` pushed to https://github.com/Guilh/gr-kart-game (public repo; the commits show the author's email). Deployed: the first `vercel deploy` went straight to production (Vercel's behaviour for a new project), so the game is public at https://gr-kart-sakura-circuit.vercel.app. The owner is fine with it being public. Open: the production deploy of the license notices (`vercel deploy --prod`, run by the owner), optionally `vercel git connect` for deploys on push, the sound check, and the GR-style logo decision. `tsc`, `npm run build` (no warnings) and the preview smoke test pass. `sim:check` wasn't needed (no physics or AI changes). Items 3.7–3.10 and sections 4, 5, 6 and 8 are in the post-launch backlog.
