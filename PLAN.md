...
- [ ] Test on a real iPad: tilt feel (deadzone and full-lock angle), zone size, sound unlock, performance on Low. (2026-09-24: the owner has played it on an iPad with the touch pad and pedals, which gave the layout feedback above; tilt and sound not yet reported.) The Browser pane can't emulate iPad Safari or real sensors, and motion access needs HTTPS, so test on the branch's Vercel preview.
